...
LORA_ADAPTER_PATH = "outputs/checkpoint-1500"
...
def is_schedule_valid(completion, original_events):
    ...
    - Events are sorted
    - No overlaps
    - All scheduled events come from the original list
    - At least 2 events are scheduled
    """
    ...
    # Check if all events are from the original list
    original_event_set = {tuple(e) for e in original_events}
    scheduled_event_tuples = {(name, start, end) for name, start, end in scheduled_events}
...
def main():
    ...
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name=LORA_ADAPTER_PATH, # Load the LoRA adapter directly
    ...
    for item in tqdm(ds, desc="Evaluating"):
        ...
        # We can still count valid schedules separately if needed
        if score > 0:
